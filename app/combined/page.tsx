...
function seriesPresentation(row: SeriesRow, locks: BetLock[]) {
  ...
  const hasHistorical = hasStarted && row.decision.probabilityA !== null && Number.isFinite(row.decision.probabilityA);
  const source = betLock ? "bet" : hasHistorical ? "historical" : "latest";
  const displayProbabilityA = betLock?.probabilityA ?? (hasHistorical ? row.decision.probabilityA! : latestProbabilityA);
  const displayWinner = betLock?.recommendedWinner ?? (hasHistorical ? row.decision.predictedWinner : latestWinner);
  const displayExact = betLock?.exactScore ?? (hasHistorical ? row.decision.predictedExactScore : latestExact?.score) ?? null;
  const displayExactProbability = betLock ? null : hasHistorical ? row.decision.predictedExactScoreProbability : latestExact?.probability ?? null;
  return { betLock, source, hasStarted, latestProbabilityA, latestWinner, latestExact, displayProbabilityA, displayWinner, displayExact, displayExactProbability };
}
