...
function mapsForSeries(row: SeriesRow, maps: MapRow[]) {
  ...
  if (canonicalSeriesId) return maps.filter((map) => String(map.seriesId) === String(canonicalSeriesId)).sort((a, b) => Number(a.startTime || 0) - Number(b.startTime || 0));
  if (row.live?.matchId) return maps.filter((map) => String(map.matchId) === String(row.live?.matchId)).sort((a, b) => Number(a.startTime || 0) - Number(b.startTime || 0));
  return [];
}
...
function SwissMatrix({ rows, maps, heroes, locks, isAdmin, locking, onLock, expanded, onToggle }: { rows: SeriesRow[]; maps: MapRow[]; heroes: Map<number, Hero>; locks: BetLock[]; isAdmin: boolean; locking: string | null; onLock: (request: LockRequest) => void; expanded: Record<string, string>; onToggle: (matchId: number) => void }) {
  ...
  return <section className="fusion-panel fusion-matrix"><header><div><span>SWISS · ОДНА СЕРИЯ = ОДНА СТРОКА</span><h2>Standings и матчи по раундам</h2><p>Таблица показывает положение команд, а ниже каждая серия встречается ровно один раз. Раскройте карточку для прогноза, ставки, карт и пиков.</p></div><div className="fusion-matrix-progress"><b>{completedCount}</b><span>завершено</span><small>из {rows.length} матчей</small></div></header>
    <div className="fusion-swiss-layout"><div className="fusion-table-wrap fusion-scroll-hint" role="region" aria-label="Таблица standings; на узком экране прокручивается горизонтально"><table className="fusion-table fusion-standings-table"><thead><tr><th>#</th><th>Команда</th><th>Группа</th><th>Матчи</th><th>Статус</th></tr></thead><tbody>{standings.map((standing, index) => <tr key={standing.teamId}><td><strong>{index + 1}</strong></td><td><Team id={standing.teamId} /></td><td><span className={`fusion-group fusion-group--${standing.group.toLowerCase()}`}>{standing.group}</span></td><td><strong>{standing.wins}–{standing.losses}</strong></td><td><span className={`fusion-outcome ${standing.wins >= 4 ? "is-qualified" : standing.losses >= 4 ? "is-eliminated" : ""}`}>{standing.wins >= 4 ? "НАПРЯМУЮ" : standing.losses >= 4 ? "ВЫЛЕТ" : standing.wins + standing.losses >= 5 ? "СТЫК" : standing.liveOpponent ? `LIVE vs ${team(standing.liveOpponent).short}` : "В ИГРЕ"}</span></td></tr>)}</tbody></table></div>
