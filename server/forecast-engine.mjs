...
const DEFAULT_CALIBRATION = Object.freeze({ liveGlobal: 0, probabilityTemperature: 1, formLogitSd: 0, seriesNoiseLogitSd: 0 });
...
export function resolveTournamentCalibration(stats) {
  const artifact = stats?.tournamentCalibration;
  const selected = { ...DEFAULT_CALIBRATION, ...(artifact?.selected ?? {}) };
  const onlinePassed = artifact?.validation?.gates?.onlineUpdate?.passed === true;
  const formPassed = artifact?.validation?.gates?.formUncertainty?.passed === true;
  return {
    ...selected,
    liveGlobal: onlinePassed ? Number(selected.liveGlobal || 0) : 0,
    formLogitSd: formPassed ? Number(selected.formLogitSd || 0) : 0,
    seriesNoiseLogitSd: 0,
    shadowLiveGlobal: Number(artifact?.shadow?.liveGlobal || 0),
    shadowFormLogitSd: Number(artifact?.shadow?.formLogitSd || 0),
  };
}
...
export function buildForecastSource({ answers, stats, matches, mode = "mixed", opinionWeight = 50 }) {
  ...
  const calibration = resolveTournamentCalibration(stats);
...
export function runForecast(answers, iterations = 100000, seed = Math.floor(Math.random() * 0xffffffff), { matches = [], stats = null, adaptive = null } = {}) {
  ...
  for (let iteration = 0; iteration < adaptiveConfig.maxIterations; iteration += 1) {
    ...
    const play = (a, b, fixed = null) => { const winner = fixed === a || fixed === b ? fixed : winnerFor(a, b); const loser = winner === a ? b : a; records[winner].wins++; records[loser].losses++; records[a].opponents.add(b); records[b].opponents.add(a); const key = pairKey(a, b); const first = key.split("|")[0]; const matchupKey = `${round}:${key}`; const matchup = swissMatchupCounts.get(matchupKey) || { count: 0, firstWins: 0 }; matchup.count++; if (winner === first) matchup.firstWins++; swissMatchupCounts.set(matchupKey, matchup); path.push(`${round}:${key}>${winner}`); };
  ...
  const swissMatchups = [...swissMatchupCounts].sort(([keyA, valueA], [keyB, valueB]) => Number(keyA.split(":")[0]) - Number(keyB.split(":")[0]) || valueB.count - valueA.count || keyA.localeCompare(keyB)).map(([key, value]) => { const separator = key.indexOf(":"); const round = Number(key.slice(0, separator)); const [a, b] = key.slice(separator + 1).split("|"); return { round, a, b, probability: 100 * value.count / completedIterations, aWinProbability: 100 * value.firstWins / value.count, occurrences: value.count }; });
