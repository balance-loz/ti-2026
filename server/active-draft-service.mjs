...
function assignmentScore(team, heroes, active, observedPlayers) {
  ...
  if (!heroes.length || players.length < heroes.length) return { rate: .5, roleRate: .5 };
  const score = (assignments) => {
    ...
    return { rate: sigmoid(pool / assignments.length), roleRate: sigmoid(roles / assignments.length) };
  };
  if (observedPlayers?.length === heroes.length) {
    ...
    if (exact.every((row) => row.player && row.hero)) return score(exact);
  }
  let best = null;
  ...
  visit(0, new Set(), [], 0);
  return best ? score(best.rows) : { rate: .5, roleRate: .5 };
}
...
function activeSignals(draftStats, teamA, teamB, radiant, heroesA, heroesB, liveGame) {
  ...
  const synergy = (heroes) => {
    ...
    return average(rows.map((row) => Number(row.coefficient ?? logit(Number(row.winRate) / 100))), 0);
  };
  ...
  const counters = heroesA.flatMap((a) => heroesB.map((b) => active?.counter?.[`${a.id}>${b.id}`] ?? draftStats.counters?.[`${a.id}|${b.id}`]).filter(Boolean));
  ...
  return [
    { key: "side", value: logit(Number(active?.radiantWinRate ?? draftStats.radiantWinRate ?? 50) / 100) * (radiant === "a" ? 1 : -1) },
    { key: "hero", value: logit(average(heroesA.map(heroRate))) - logit(average(heroesB.map(heroRate))) },
    { key: "draftPriority", value: average(priority(heroesA).map((row) => Number(row.score)), 0) - average(priority(heroesB).map((row) => Number(row.score)), 0) },
    { key: "synergy", value: synergy(heroesA) - synergy(heroesB) },
    { key: "counter", value: average(counters.map((row) => Number(row.coefficient ?? logit(Number(row.winRate) / 100))), 0) },
    { key: "teamPool", value: logit(familiarity(teamA, heroesA)) - logit(familiarity(teamB, heroesB)) },
    { key: "playerPool", value: logit(assignmentA.rate) - logit(assignmentB.rate) },
    { key: "roles", value: logit(assignmentA.roleRate) - logit(assignmentB.roleRate) },
  ];
}
...
export function calculateActiveDraftPrediction({ draftStats, teamStats, game } = {}) {
  ...
  const signals = activeSignals(draftStats, game.radiantTeam, game.direTeam, "a", heroesA, heroesB, game);
