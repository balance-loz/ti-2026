...
export const JOB_LABELS = {
  ...
  freeze: ["Фиксация до старта", "Записывает прогноз до начала матча по расписанию организатора"],
...
export function describeJobRun(job, detail) {
  ...
  switch (job) {
    ...
    case "freeze": {
      const parts = [];
      if (detail.frozen) parts.push(`зафиксировано ${count(detail.frozen, "прогноз", "прогноза", "прогнозов")} до старта`);
      if (detail.linked) parts.push(`привязано к сыгранным сериям ${detail.linked}`);
      return parts.join(", ") || "нечего фиксировать";
    }
    case "resolve":
