...
def main():
    ...
    con.close(); team_oof=load_team_oof(); rows=[]; rejected=defaultdict(int)
    for m in matches:
        ...
        radiant_team=int(m['radiant_team_id']); p=prior['probabilityA'] if radiant_team==prior['teamA'] else 1-prior['probabilityA']
        objectives=raw.get('objectives') or []
        row={'matchId':match_id,'seriesId':str(m['series_id']),'startTime':int(m['start_time']),'leagueId':int(m['league_id']),'patchId':int(m['patch_id']),'subpatchId':str(m['subpatch_id'] or m['patch_id']),'radiantTeamId':radiant_team,'direTeamId':int(m['dire_team_id']),'preDraftProbability':max(.01,min(.99,p)),'teamUncertainty':prior['playerVariance'],'radiantWin':int(m['radiant_win']),'duration':int(raw.get('duration') or m['duration'] or 0),'gold10':minute(raw.get('radiant_gold_adv'),10),'gold15':minute(raw.get('radiant_gold_adv'),15),'gold20':minute(raw.get('radiant_gold_adv'),20),'xp10':minute(raw.get('radiant_xp_adv'),10),'firstBlood':objective_side(objectives,'firstBlood'),'firstTower':objective_side(objectives,'firstTower'),'firstRoshan':objective_side(objectives,'firstRoshan'),'players':roster,'draftSequence':seq,'source':str(m['source']),'parseQuality':float(m['parse_quality'] or 0),'checksum':str(m['content_checksum'])}
        rows.append(row)
    OUTPUT.write_text('\n'.join(json.dumps(r,separators=(',',':')) for r in rows)+'\n',encoding='utf8')
    target_names=['radiantWin','gold10','gold15','gold20','xp10','duration','firstBlood','firstTower','firstRoshan']
    report={'schemaVersion':1,'population':'parsed pro maps with exactly ten picks and complete event/roster data','maps':len(rows),'series':len({r['seriesId'] for r in rows}),'patches':len({r['subpatchId'] for r in rows}),'leagues':len({r['leagueId'] for r in rows}),'fullSequenceMaps':sum(len(r['draftSequence'])>=20 for r in rows),'eventCount':sum(len(r['draftSequence']) for r in rows),'targetCoverage':{key:sum(r[key] is not None for r in rows) for key in target_names},'rejected':dict(rejected),'timeRange':[rows[0]['startTime'],rows[-1]['startTime']] if rows else None,'leakageContract':'preDraftProbability is emitted before the containing series update; all post-draft cross-fitting must use earlier maps only'}
