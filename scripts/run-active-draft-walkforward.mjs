...
const FEATURE_NAMES = ["teamPrior", "side", "hero", "synergy", "counter", "teamPool", "playerPool", "roles"];
const PRIORS = { team: 12, side: 30, hero: 18, pair: 24, pool: 6, role: 12 };
...
function freshState(teamModelDefinition) {
  return { maps: 0, radiantWins: 0, teamModel: createOnlineTeamModel(teamModelDefinition), hero: new Map(), synergy: new Map(), counter: new Map(), teamHero: new Map(), playerHero: new Map(), heroRole: new Map(), playerRole: new Map() };
}
...
function features(state, match) {
  ...
  return [teamPrior, side, hero, synergy, counter, teamPool, playerPool, roles];
}
...
function fitFrozenCombiner(rows, activeIndices = FEATURE_NAMES.map((_, index) => index)) {
  ...
  for (let iteration = 0; iteration < 900; iteration += 1) {
    ...
    fitted = fitted.map((weight, index) => activeIndices.includes(index) ? clamp(weight - learningRate * gradient[index], 0, 2.5) : 0);
  }
  return fitted;
}
...
let weights = [1, .25, .25, .12, .12, .08, .08, .08]; const rows = [];
...
for (const match of matches) {
  ...
  weights = weights.map((weight, index) => clamp(weight - rate * (error * x[index] + .002 * weight), index === 0 ? 0 : -.5, 2.5));
...
for (const row of metaTest) { row.frozenModel = probabilityWithWeights(row, frozenWeights); row.frozenTeamSide = probabilityWithWeights(row, frozenTeamSideWeights); }
const productionWeights = fitFrozenCombiner(rows);
...
const combiner = {
  version: 2, learnedFrom: "batch logistic combiner fitted only on chronological OOF map features; validated on a frozen final 30% time holdout", featureOrder: FEATURE_NAMES,
...
stats.activeSnapshot = {
  ...
  teamPairwise: Object.fromEntries(Object.entries(teamStats.pairwise ?? {}).map(([key, pair]) => [key, { probabilityA: Number(pair.mapProbabilityA ?? 50), modelId: pair.modelId ?? teamStats.methodology?.teamPrior?.modelId ?? null }])), hero: serialize(state.hero, PRIORS.hero), synergy: Object.fromEntries([...state.synergy.keys()].map((key) => [key, residualSample(state.synergy, key)])), counter: Object.fromEntries([...state.counter.keys()].map((key) => [key, residualSample(state.counter, key)])),
