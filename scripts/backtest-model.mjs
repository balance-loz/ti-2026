...
function predictWithHistory(history, test, halfLife) {
  ...
  for (let iteration = 0; iteration < 180; iteration += 1) {
    const gradient = Object.fromEntries(Object.keys(ratings).map((id) => [id, -0.04 * ratings[id]]));
    ...
    for (const id of Object.keys(ratings)) ratings[id] += 0.025 * gradient[id];
  }
...
const report = { generatedAt: new Date().toISOString(), method: "expanding-window walk-forward; every prediction uses only earlier series", chosenHalfLifeDays: results[0]?.halfLifeDays ?? 75, candidates: results };
