...
Прогноз становится ставкой только после явного действия администратора «Я поставил по рекомендации». В этот момент `bet_locks` неизменяемо сохраняет выбранную моделью сторону, вероятность, точный счёт, профиль, snapshot, пики и время. Пока кнопка не нажата, рекомендация может обновляться; после нажатия новая информация отображается как `LATEST`, но не переписывает ставку. Для серии и каждой карты после драфта используются независимые locks. Повторная фиксация и фиксация после известного результата запрещены API.
