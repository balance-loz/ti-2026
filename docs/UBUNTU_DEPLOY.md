...
AUTO_SNAPSHOT_ITERATIONS=100000
...
API раз в 10 минут делает один запрос списка матчей лиги `19719` в OpenDota. Завершённые карты объединяются по `series_id`; BO3 записывается после двух побед, BO5 — после трёх. После нового результата сервер самостоятельно выполняет 100 000 прогонов и сохраняет снимок истории. Кнопка «Проверить результаты TI» запускает такую же проверку вручную.
